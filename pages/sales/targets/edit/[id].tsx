...
const EditTargetPage: React.FC = () => {
        ...
        try {
          ...
          if (response.data.success && response.data.data) {
            ...
            setFormData({
              ...
              target_income: targetData.target_income.toString(),
              sort: targetData.sort || 0
            });
          } else {
            setError('获取销售目标详情失败');
          }
        } catch (err) {
          setError('获取销售目标详情失败，请稍后再试');
        } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || '更新销售目标失败');
      }
    } catch (err) {
      setError('更新销售目标失败，请稍后再试');
    } finally {
  ...
  return (
    <Layout>
      <Head>
        <title>编辑销售目标 | 销售助手</title>
        <meta name="description" content="编辑销售目标" />
      </Head>

      <div className="py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-gray-900">编辑销售目标</h1>
          <button
            onClick={() => router.back()}
